...
- 迷惑メール登録を選んだ場合: 既存の該当メールをGmailの迷惑メールへ移動する
- 削除だけを選んだ場合: 既存の該当メールをGmailのゴミ箱へ移動する
...
Gmail由来の対象を選択した場合は、確認ダイアログで「迷惑メールに登録」または「削除だけ」を選べます。実際の購読解除を行う場合は、`List-Unsubscribe` ヘッダーの内容を解析し、解除先URLまたはメール送信先をユーザーに明示したうえで、最終確認を追加してから実行する設計にします。
